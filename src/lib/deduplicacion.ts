...
export async function deduplicarMencion(
  ...
  const candidatos = await db.mencion.findMany({
    where: {
  ...
  console.log(`[DEDUP] Evaluando medio ${input.medioId}... ${candidados.length} candidatos encontrados. Huella: ${huella}`);
