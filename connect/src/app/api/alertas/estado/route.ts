...
export async function GET() {
  try {
    ...
    for (const ind of indicadoresDB) {
      if (ind.valores.length === 0) continue;
      ...
      const historial = ind.valores.map(v => ({
        fecha: v.fecha,
        valor: v.valor,
      }));
      ...
      if (umbralSlug === 'rin_variacion_semanal_mm_usd' && ind.valores.length >= 2) {
        // RIN: calcular variación entre el último y el penúltimo
        valor = ind.valores[0].valor - ind.valores[1].valor;
      } else if (umbralSlug === 'volatilidad_minera_pct_semanal' && ind.valores.length >= 2) {
        // Minerales: calcular variación porcentual semanal
        const anterior = ind.valores[1].valor;
        if (anterior !== 0) {
          valor = Math.abs((ultimo.valor - anterior) / anterior * 100);
        }
      } else if (umbralSlug === 'litio_precio_variacion_pct' && ind.valores.length >= 2) {
        const anterior = ind.valores[1].valor;
        if (anterior !== 0) {
          valor = Math.abs((ultimo.valor - anterior) / anterior * 100);
        }
